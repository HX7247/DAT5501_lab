...
def _regression_stats(x, y):
    # Return simple linear regression stats: slope, intercept, R, R²
    if len(x) < 2:
...
def plot_meat_vs_gdp(df, show=True, save_path=None):
    ...
    # Color points by region (if available)
    if region_col:
    ...
    # Per-region regression summaries (not plotted)
    if region_col:
        per_region_texts = []
        for reg in regions:
...
def plot_region_averages(df, show=True, save_path=None):
    ...
    # Prepare colors for regions
    regions = list(region_means[region_col])
    cmap = plt.get_cmap("tab20")
    colors = cmap(np.linspace(0, 1, len(regions)))
    color_map = dict(zip(regions, colors))

    # Extract full dataset values
    x_all = df_plot[gdp_col].values
    y_all = df_plot[meat_col].values

    plt.figure(figsize=(10, 7))

    # Plot all countries as grey background points
    plt.scatter(x_all, y_all, color="#cccccc", alpha=0.5, s=30,
                edgecolors='none', label="Countries")
    ...
    # Legend: countries + region colors
    region_patches = [mpatches.Patch(color=color_map[r], label=r) for r in regions]
    country_handle = Line2D([0], [0], marker='o', color='w',
                            label='Countries', markerfacecolor='#cccccc', markersize=6)
...
if __name__ == "__main__":
    # Run plots if executed directly
    df = load_data()
    plot_meat_vs_gdp(df)
